...
export function WeddingTemplate({ variant, data, slug, guestToken, demo = false }: {
  ...
  return <main className={`${styles.page} ${styles[variant]}`}>
    ...
    <section id="story" className={styles.story}>
      <div><p className={styles.eyebrow}>01 / Our story</p><h2>{data.story.title}</h2><p>{data.story.body}</p><p className={styles.script}>{bride} & {groom}</p></div>
      <figure><Photo src={data.gallery[1] ?? data.gallery[0]} alt="Kenangan perjalanan kami" /><figcaption>Little moments, a lifetime of memories.</figcaption></figure>
    </section>
    <div className={styles.countdown}><CountdownSection data={data} /></div>
    <section id="day" className={styles.day}>
      <div className={styles.sectionHeading}><p className={styles.eyebrow}>02 / The wedding day</p><h2>Hari yang kami nantikan.<br /><em>Bersama kalian.</em></h2><p>{date} · {data.place}</p></div>
      <div className={styles.events}>{data.events.map((event, index) => <article key={`${event.name}-${index}`}><span className={styles.eyebrow}>0{index + 1}</span><h3>{event.name}</h3><p>{dateLabel(event.date)}</p><p>{event.time}</p><p>{event.venue}</p></article>)}</div>
      <div className={styles.location}><div><p className={styles.eyebrow}>Meet us here</p><h3>{data.place}</h3><p>{data.address}</p></div>{data.mapsUrl && /^https?:\/\//i.test(data.mapsUrl) && <a className={styles.button} href={data.mapsUrl} target="_blank" rel="noreferrer">Petunjuk lokasi ↗</a>}</div>
    </section>
    {data.gallery.length > 0 && <section id="memories" className={styles.memories}><div className={styles.sectionHeading}><p className={styles.eyebrow}>03 / In good company</p><h2>Momen kecil,<br /><em>kenangan selamanya.</em></h2></div><div className={styles.gallery}>{data.gallery.map((src, index) => <figure key={`${src}-${index}`}><Photo src={src} alt={`Kenangan ${bride} dan ${groom}, foto ${index + 1}`} /><figcaption>({String(index + 1).padStart(2, "0")}) A moment to keep</figcaption></figure>)}</div></section>}
    <div id="rsvp" className={styles.response}><RSVPSection slug={slug} guestToken={guestToken} demo={demo} /></div>
    <div className={styles.gifts}><GiftSection data={data} /></div>
    <footer className={styles.footer}><p className={styles.eyebrow}>The beginning of forever</p><h2>{bride} <em>&</em> {groom}</h2><p>Terima kasih sudah menjadi bagian dari cerita kami.</p><div><span>{date}</span><a href="#home">Kembali ke atas ↑</a><Link href="/">Made with love · Yukakad</Link></div></footer>
  </main>;
}
